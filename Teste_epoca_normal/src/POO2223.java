...
public class POO2223 {
	...
	private static void alinea2(PrintStream out) throws FileNotFoundException {
		...
		*/
		...
		Scanner file = new Scanner(new File("bookings.txt"));
		file.nextLine();
		file.nextLine();

		// 123456789;2023-06-01;150;WAREHOUSE;200x300x250
		// 501234567;2023-06-05;100;GARAGE;250x400x200
		// 150987654;2023-06-10;20;LOCKER;50x50x50
		// 150987654;2023-06-20;360;GARAGE;200x300x260

		while (file.hasNextLine()) {
			String[] line = file.nextLine().split(";");
			Client client = storageService.getClient(Integer.parseInt(line[0]));
			LocalDate date = LocalDate.parse(line[1]);
			int duration = Integer.parseInt(line[2]);
			Type type = null;
			if (line[3].equals("WAREHOUSE")) {
				type = Type.WarehouseStorage;
			} else if (line[3].equals("GARAGE")) {
				type = Type.GarageStorage;
			} else if (line[3].equals("LOCKER")) {
				type = Type.LockerStorage;
			}
			int[] dimensions = new int[3];
			String[] dim = line[4].split("x");
			for (int i = 0; i < 3; i++) {
				dimensions[i] = Integer.parseInt(dim[i]);
			}
			List<StorageUnit> unit = storageService.findAvailableUnits(type, date, duration, dimensions);
			if (unit != null && !unit.isEmpty()) {
				storageService.rentStorageUnit(client, unit.get(0), date, duration);
			}
		}
		...
		*/
		...
		List<StorageUnit> available = storageService.findAvailableUnits(Type.GarageStorage, LocalDate.of(2023, 6, 1),90, new int[]{300, 400, 300});
		...
		available = storageService.findAvailableUnits(Type.GarageStorage, LocalDate.of(2023, 6, 1),90, new int[]{300, 400, 280});
		if (available==null || available.isEmpty()) {
			out.println("** No options available for the type, dimensions, and dates requested.\n");
		} else {
			StorageUnit u = available.get(0);
